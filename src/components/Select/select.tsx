...
export const Select: FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames("sweet-select", {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="sweet-select-input" onClick={handleClick}>
        <Input
          // ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition in={menuOpen} animation="zoom-in-top" timeout={300}>
          <ul className="sweet-select-dropdown">{generateOptions()}</ul>
        </Transition>
      </SelectContext.Provider>
      {multiple && (
        <div
          className="sweet-selected-tags"
          style={{ maxWidth: containerWidth.current - 32 }}
        >
          {selectedValues.map((value, index) => {
            return (
              <span className="sweet-tag" key={`tag-${index}`}>
                {value}
                <Icon
                  icon="times"
                  onClick={() => {
